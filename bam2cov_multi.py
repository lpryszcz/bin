...
def worker(inQ, output, c2i, bufferSize):
    ...
    for blocks, strands, pos, aend, rname in iter(inQ.get, 'STOP'):
        ...
        # send to queue
        output.put((cminus, cplus, list(selected)))
    #send stop
    output.put('STOP')
        
def alignment_iterator(queue, bam, mapq, nprocs, verbose):
    ...
    for i, a in enumerate(sam, 1):
        ...
        if pa.pos==a.pos and pa.cigarstring==a.cigarstring:
            ...
            queue.put((pa.blocks, strands, pa.pos, pa.aend, sam.references[pa.rname]))
            # store current entry
            pa = a
...
def parse_bam(bam, mapq, c2i, entries, bufferSize, verbose, nprocs=4):
    ...
    workers = []
    for i in range(nprocs):
        p = Process(target=worker, args=(inQ, outQ, c2i, bufferSize))
        p.start()
        workers.append(p)
    # parse results
    i = 1
    for procid in range(1, nprocs+1):
        for i, (cminus, cplus, selected) in enumerate(iter(outQ.get, 'STOP'), i):
            # store info
