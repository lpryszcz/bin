...
import argparse, MySQLdb, os, sys
...
def taxid2proteomes(m, name, verbose): #taxa, group_taxid, verbose):
    """Fetch proteomes for given taxid"""
    #get group info
    #parent_id, name, rank = taxa.get_taxa_info(group_taxid)
    outdir = name #"%s_%s"%(group_taxid, name)
    ...
    if verbose:
        #info = "Fetching %s taxa for %s [%s %s]...\n"
