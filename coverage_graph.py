...
def record2graph( fnames,beds,r,expcounts,minlog,window,printMerged,verbose ):
    """ """
    #create diagram
    gdd  = GenomeDiagram.Diagram() #GDDiagram(gb)
    #add annotation
    gdt1 = gdd.new_track( 1,greytrack=1,name="%s: Genes & GC" % r.id,height=1.5,scale_smalltick_interval=5*10**4,scale_smallticks=0.15,scale_largetick_labels=0,scale_fontangle=0 )
    ...
    #get gc graph
    gcgraph = seq2gcgraph( r,beds[0] )
    gdgs.new_graph( gcgraph,"GC content",style="line",colour=colors.blue,center=50 )

    #add merged coverage track
    colorCats = COLORCATS
    basei     = 3
    linewidth = 2
    height    = 1
    if len(fnames)<=12 and printMerged:
        gdtm   = gdd.new_track( 2,greytrack=1,name="MERGED chromosome: %s" % r.name,height=1.5,scale_smalltick_interval=5*10**4,scale_smallticks=0.15,scale_largeticks=1.0,scale_largetick_labels=1,scale_largetick_interval=250*10**3,scale_fontangle=0 ) #,scale_font=3 )
        gdgsm  = gdtm.new_set("graph")
    else:
        colorCats = [ (0,0,1) for fn in fnames ]
        basei     = 2
        linewidth = 1
        height    = 1.5
    ...
    for bed,fn,expcount in zip( beds,fnames,expcounts ):
        ...
        #add to merged
        if len(fnames)<=12 and printMerged:
            gdggm = gdgsm.new_graph( gdata,fn,style="line",linewidth=1,center=0,color=colors.Color(colorCats[i][0],colorCats[i][1],colorCats[i][2]) ) #,altcolor="blue",
        ...
        gdgg  = gdgs.new_graph( gdata,fn,style="line",linewidth=linewidth,center=0,color=colors.Color(colorCats[i][0],colorCats[i][1],colorCats[i][2]) ) #,altcolor="blue",
        clen  = gdgg.range()[1]
        i    += 1
...
def coverage_graph( outdir,fnames,gfn,fnformat,gff,lenlimit,ext,minlog,minreads,window,printMerged,verbose ):
    ...
    for r in seqobjects:
        ...
        if len(r.seq) >= lenlimit*10**3:
            ...
            gdd   = record2graph( fnames,counts,r,expcounts,minlog,window,printMerged,verbose )
            #save
...
def main():
    ...
    parser.add_option("-m", dest="minlog",   default=-4, type=int,
                      help="min log2 value      [%default kb]" )
    ...
    parser.add_option("-p", dest="printMerged", default=True, action="store_false",
                      help="print merged track  [True]" )
    ...
    #elif len(args)>len(colorCats):
    #    parser.error( "At max %s BED files allowed!" % len(colorCats) )
    ...
    #plot
    coverage_graph( o.outdir,args,o.genome,o.format,o.gff,o.lenlimit,o.ext,o.minlog,o.minreads,o.window,o.printMerged,o.verbose )
